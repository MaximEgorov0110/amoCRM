define(['jquery'], function ($) {
	return function () {
		...
		// Создаем фунциию коприрования
		const copyLead = (leadId, pipelineId, statusId) => {
			return new Promise((resolve, reject) => {
				// 1. Получаем данные текущей сделки с контактами
				$.ajax({
					method: 'GET',
					...
					success: function (leadData) {
						...
						// 2. Подготавливаем данные для новой сделки
						const newLeadData = {
							...
							_embedded: {
								tags: originalLead._embedded?.tags || []
							}
						};

						// 3. Создаем новую сделку
						$.ajax({
							method: 'POST',
							url: '/api/v4/leads',
							contentType: 'application/json',
							data: JSON.stringify([newLeadData]),
							success: function (newLeadResponse) {
								const newLeadId = newLeadResponse._embedded.leads[0].id;

								// 4. Если есть контакты, прикрепляем их к новой сделке
								if (originalLead._embedded && originalLead._embedded.contacts) {
									const contacts = originalLead._embedded.contacts;
									const linkRequests = contacts.map(contact => {
										return $.ajax({
											method: 'POST',
									...
									// Ждем завершения всех запросов для добавления контактов
									Promise.all(linkRequests)
										.then(() => resolve(newLeadId))
										.catch(reject);
								} else {
									resolve(newLeadId);
								}
			...
			render: function () {
				...
				$('.copy-lead__button').on('click', function () {
					if (!$(this).hasClass('copy-lead__button_disable')) {
						...
						if (success) {
							copyLead(lead_id, pipeline_id, status_id)  // Вызываем функцию
							$('.copy-lead__info')
